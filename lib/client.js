...
function UpdateDialog({ children, onClose }) {
  ...
  const descriptionId = React20.useId();
  React20.useEffect(() => {
  ...
  return typeof document !== "undefined" && document.body ? (0, import_react_dom2.createPortal)(content, document.body) : content;
}
function UpdatePanel({ rpcCall, clientVersion, onStatus }) {
  const [snapshot, setSnapshot] = React20.useState(null);
  const [action, setAction] = React20.useState("status");
  const [error, setError] = React20.useState(null);
  const [open, setOpen] = React20.useState(false);
  const [uncertainInstall, setUncertainInstall] = React20.useState(false);
  const mounted = React20.useRef(false);
  const busy = React20.useRef(false);
  const readController = React20.useRef(null);
  const pollReadController = React20.useRef(null);
  const installRequest = React20.useRef(null);
  const onStatusRef = React20.useRef(onStatus);
  onStatusRef.current = onStatus;
  const accept = React20.useCallback((next) => {
    setSnapshot(next);
...
function IMSettingsTab({
  ...
  const [selected, setSelected] = React21.useState("weixin");
  const [loopbackRecovery, setLoopbackRecovery] = React21.useState(null);
  const [runningVersion, setRunningVersion] = React21.useState(IM_PLUGIN_VERSION);
  const githubTooltipId = React21.useId();
  const active = CHANNELS.find((channel4) => channel4.id === selected) ?? CHANNELS[0];
  const reportLoopbackRecovery = React21.useCallback((recovery) => {
    setLoopbackRecovery((current) => current?.url === recovery.url ? current : recovery);
  }, []);
  const reportUpdateStatus = React21.useCallback((snapshot) => {
    setRunningVersion(snapshot.runningVersion);
  }, []);
  const rpcCalls = React21.useMemo(() => createLoopbackAwareRpcCalls({
    dingtalkRpcCall,
    discordRpcCall,
    feishuRpcCall,
    qqRpcCall,
