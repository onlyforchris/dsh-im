...
var EN = Object.freeze({
  ...
  "\u672A\u914D\u7F6E": "Not configured",
  "\u5DF2\u8FDE\u63A5 ${statusCounts.connected} \xB7 \u672A\u8FDE\u63A5 ${statusCounts.offline} \xB7 \u672A\u914D\u7F6E ${unconfigured}": "${statusCounts.connected} connected \xB7 ${statusCounts.offline} offline \xB7 ${unconfigured} not configured",
  "\u5FAE\u4FE1": "WeChat",
  "\u98DE\u4E66": "Feishu",
  "\u9489\u9489": "DingTalk",
  "\u4F01\u4E1A\u5FAE\u4FE1": "WeCom",
...
function translateDynamic(text5) {
  let match = /^已连接 (\d+) · 未连接 (\d+) · 未配置 (\d+)$/.exec(text5);
  if (match) return `${match[1]} connected \xB7 ${match[2]} offline \xB7 ${match[3]} not configured`;
  match = /^(\d+) \/ (\d+) 在线$/.exec(text5);
...
.dim-layout {
  display: grid;
  grid-template-columns: 174px 1px minmax(0, 1fr);
  gap: 24px;
  align-items: stretch;
}
.dim-rail {
...
.dim-channelStatus { width: 8px; height: 8px; border-radius: 999px; flex: none; background: var(--dsw-alias-border-l2, #dfe1e5); }
.dim-channelStatus-connected { background: var(--dsw-alias-state-success-primary, #20a162); }
.dim-channelStatus-offline { background: var(--dsw-alias-state-warn-primary, #d97706); }
.dim-channelStatus-unconfigured { background: var(--dsw-alias-border-l2, #dfe1e5); }
.dim-channelStatus-unknown { background: var(--dsw-alias-border-l2, #dfe1e5); }
.dim-channelSummary { display: flex; gap: 4px; }
...
.dim-divider {
  width: 1px;
  align-self: stretch;
  background: var(--dsw-alias-border-l1, #eef0f3);
}
...
@media (max-width: 840px) {
  ...
  .dim-rail { grid-template-columns: repeat(2, minmax(0, 1fr)); padding: 1px; }
...
var CHANNEL_STATUS_LABELS = Object.freeze({
  connected: "\u5DF2\u8FDE\u63A5",
  offline: "\u5DF2\u914D\u7F6E\uFF0C\u672A\u8FDE\u63A5",
  unconfigured: "\u672A\u914D\u7F6E",
  unknown: "\u72B6\u6001\u672A\u77E5"
...
function useChannelStatuses(rpcCalls) {
  const [statuses, setStatuses] = React18.useState({});
  React18.useEffect(() => {
    let disposed = false;
    const fetchAll = async () => {
      const entries = await Promise.all(CHANNELS.map(async (channel4) => {
        const rpcCall = rpcCalls[channel4.id];
        if (typeof rpcCall !== "function") return [channel4.id, "unknown"];
        try {
          const snapshot = unwrapStatusValue(await rpcCall("connection.status", {}));
          return [channel4.id, channelStatusFromSnapshot(snapshot)];
        } catch {
          return [channel4.id, "unknown"];
        }
      }));
      if (!disposed) setStatuses(Object.fromEntries(entries));
    };
    void fetchAll();
    const timer = setInterval(fetchAll, 15e3);
    return () => {
      disposed = true;
      clearInterval(timer);
    };
...
function IMSettingsTab({
  ...
  const [selected, setSelected] = React18.useState("feishu");
  ...
  const statusCounts = CHANNELS.reduce((counts, channel4) => {
    const state = statuses[channel4.id];
    ...
    h2(
      ...
      h2(
        ...
        h2(
          ...
          h2(
            "p",
            { className: "dim-channelSummary", role: "status" },
            `\u5DF2\u8FDE\u63A5 ${statusCounts.connected} \xB7 \u672A\u8FDE\u63A5 ${statusCounts.offline} \xB7 \u672A\u914D\u7F6E ${unconfigured}`
          )
        ),
        h2(
          "span",
      ...
      h2(
        ...
        h2(
          ...
          CHANNELS.map((channel4) => h2(
            ...
            h2(ChannelStatusDot, { state: statuses[channel4.id] })
          ))
        ),
