...
var package_default = {
  ...
  author: {
    name: "onlyforchris",
    url: "https://github.com/onlyforchris"
  },
  contributors: [
    {
      name: "xmanrui",
  ...
  repository: {
    type: "git",
    url: "git+https://github.com/onlyforchris/dsh-im.git"
  },
  homepage: "https://github.com/onlyforchris/dsh-im#readme",
  bugs: "https://github.com/onlyforchris/dsh-im/issues",
  publishConfig: {
    access: "public"
  },
  bin: {
  ...
  dsh: {
    ...
    compatibility: {
      dsh: "0.1.2-alpha.4 || 0.1.2-alpha.5 || 0.1.2-rc.1 || 0.1.3-alpha.1 || 0.1.5-alpha.1 || 0.1.5-rc.1 || 0.1.5-rc.2",
      dshReleases: {
...
function CredentialBindingPanel({
  ...
  const [identity, setIdentity] = React5.useState("");
  const [secret, setSecret] = React5.useState("");
...
function FolderIcon() {
  return React7.createElement(
    ...
    React7.createElement("path", { d: "M3.5 7.25A2.25 2.25 0 0 1 5.75 5h4.1l1.8 2h6.6a2.25 2.25 0 0 1 2.25 2.25v7A2.75 2.75 0 0 1 17.75 19h-12A2.25 2.25 0 0 1 3.5 16.75v-9.5Z" })
  );
}
function ChevronIcon() {
  return React7.createElement("svg", {
    ...
    strokeLinejoin: "round",
    "aria-hidden": "true"
  }, React7.createElement("path", { d: "m7.5 4.5 5 5.5-5 5.5" }));
}
...
function WorkspaceDirectoryPicker({
  ...
  const [listing, setListing] = React7.useState(null);
  const [loading, setLoading] = React7.useState(false);
  const [error, setError] = React7.useState(null);
  const [pathDraft, setPathDraft] = React7.useState(startPath ?? "");
  const [showHidden, setShowHidden] = React7.useState(false);
  const [retryKey, setRetryKey] = React7.useState(0);
  const requestRef = React7.useRef(0);
  const controllerRef = React7.useRef(null);
  const dialogRef = React7.useRef(null);
  const bodyRef = React7.useRef(null);
  const titleId = React7.useId();
  const noticeId = React7.useId();
  const pathInputId = React7.useId();
  const errorId = React7.useId();
  const initialPathRef = React7.useRef(startPath);
  const onPickedRef = React7.useRef(onPicked);
  const onCancelRef = React7.useRef(onCancel);
  const busyRef = React7.useRef(busy);
  onPickedRef.current = onPicked;
  onCancelRef.current = onCancel;
  busyRef.current = busy;
  const loadDirectory = React7.useCallback(async (path, { reportError = true } = {}) => {
      ...
      h2(
        ...
        loading && !listing ? h2(
          ...
          React7.createElement(
            ...
            React7.createElement("span", { className: "dim-directoryName" }, entry.name),
...
// plugin-src/client/workspace-editor.js
var WorkspaceDirectoryPickerContext = React8.createContext(null);
function WorkspaceEditor({ workspace, directoryPicker, disabled = false, onSave }) {
  const sharedDirectoryPicker = React8.useContext(WorkspaceDirectoryPickerContext);
  ...
  const editButtonRef = React8.useRef(null);
  const savingRef = React8.useRef(false);
  const close = React8.useCallback(() => {
    setOpen(false);
    setError(null);
    queueMicrotask(() => editButtonRef.current?.focus?.());
  }, []);
  const pick = React8.useCallback(async (value) => {
  ...
  return h2(
    ...
    workspace ? React8.createElement("code", {
...
function ContextEnhancementEditor({ config, groupSupported = true, disabled = false, onSave }) {
  ...
  const entryRef = React9.useRef(null);
  const dialogId = React9.useId();
  const statusId = React9.useId();
...
function useWorkspaceSnapshotFence() {
  const state = React10.useRef({ version: 0, pendingMutations: 0 });
  return React10.useMemo(() => Object.freeze({
    beginStatus() {
...
var BotSettingsContext = React11.createContext(Object.freeze({
  openBotSettings() {
  }
}));
...
function BotSettingsButton({
  ...
  const { openBotSettings } = React11.useContext(BotSettingsContext);
...
var DINGTALK_STYLE_ID = "onlyforchris-dsh-im-dingtalk-settings";
var CSS = String.raw`
.ddt-page {
  --ddt-accent: #1677ff;
...
var Button = React12.forwardRef(function Button2({ children, kind = "secondary", className = "", ...props }, ref) {
  return h2("button", {
    ...props,
    ref,
    type: "button",
...
function ConnectionErrorDiagnostic({ error }) {
  ...
  return h2(
    ...
    h2(
      ...
      h2("span", null, "\u9519\u8BEF\u7801"),
      `: ${error.code}`,
      error.referenceId ? h2(React12.Fragment, null, " \xB7 ", h2("span", null, "\u53C2\u8003\u53F7"), `: ${error.referenceId}`) : null
    )
  );
}
...
function DingtalkSettingsTab({ rpcCall }) {
  ...
  const statusRequestRef = React12.useRef(0);
  const workspaceFence = useWorkspaceSnapshotFence();
  const noticeFrameRef = React12.useRef(null);
  const focusFrameRef = React12.useRef(null);
  React12.useEffect(() => {
    mountedRef.current = true;
    return () => {
      mountedRef.current = false;
  ...
  React12.useEffect(() => installDingtalkStyles(), []);
  ...
  const discardStaleFeedback = React12.useCallback((snapshot) => {
  ...
  const focusAddButton = React12.useCallback(() => {
    if (!mountedRef.current) return;
  ...
  const setBotBusy = React12.useCallback((botId, operation) => {
    if (!mountedRef.current) return;
  ...
  const runBotAction = React12.useCallback(async ({ account, operation, endpoint, payload, success }) => {
  ...
  const reconnect = React12.useCallback((account) => runBotAction({
    account,
    operation: "reconnect",
  ...
  const saveBotSetting = React12.useCallback(async (account, operation, endpoint, payload) => {
    const snapshotVersion = workspaceFence.beginMutation();
    setBotBusy(account.botId, operation);
    try {
      const snapshot = normalizeSnapshot(await invoke(
...
var Button3 = React13.forwardRef(function Button4({ children, kind = "secondary", className = "", ...props }, ref) {
  return h2("button", {
    ...props,
    ref,
    type: "button",
...
function createTokenChannelSettings(definition) {
  ...
  function SettingsTab({ rpcCall }) {
    ...
    const [testNoticeByBot, setTestNoticeByBot] = React13.useState({});
    ...
    const bindCredentials = React13.useCallback(async (values) => {
      const snapshotVersion = workspaceFence.beginMutation();
      setBusy(true);
      setCredentialError(null);
      try {
...
var DISCORD_STYLE_ID = "onlyforchris-dsh-im-discord-settings";
var CSS2 = String.raw`
...
function useAnimationFrameScheduler() {
  const schedulerRef = React14.useRef(null);
  React14.useEffect(() => {
...
var Button5 = React15.forwardRef(function Button6({ children, kind = "secondary", size, icon, className = "", ...props }, ref) {
  return h2("button", {
    ...props,
    ref,
    type: "button",
...
function QrPane({ provision, now, onRefresh, onCancel, busy }) {
  ...
  React15.useEffect(() => setImageFailed(false), [qrSource]);
  return h2(
    "div",
...
function RemoveConfirmation2({ bot, busy, onConfirm, onCancel }) {
  const cancelRef = React15.useRef(null);
  ...
  React15.useEffect(() => cancelRef.current?.focus(), []);
  return h2(
    "div",
    {
...
function BotCard({
  ...
  const repairTooltipId = React15.useId();
...
function FeishuSettingsTab({ rpcCall }) {
  ...
  const [pageBusy, setPageBusy] = React15.useState(false);
  const [provisionBusy, setProvisionBusy] = React15.useState(false);
  const [credentialOpen, setCredentialOpen] = React15.useState(false);
  const [credentialBusy, setCredentialBusy] = React15.useState(false);
  const [credentialError, setCredentialError] = React15.useState(null);
  const [busyByBot, setBusyByBot] = React15.useState({});
  const [errorsByBot, setErrorsByBot] = React15.useState({});
  const [testNoticesByBot, setTestNoticesByBot] = React15.useState({});
  const [removeTargetId, setRemoveTargetId] = React15.useState(null);
  const [announcement, setAnnouncement] = React15.useState("");
  const [now, setNow] = React15.useState(() => Date.now());
  const [focusBotId, setFocusBotId] = React15.useState(null);
  const cardRefs = React15.useRef(/* @__PURE__ */ new Map());
  const removeButtonRefs = React15.useRef(/* @__PURE__ */ new Map());
  const targetedProvisionRef = React15.useRef(null);
  ...
  const mergeSnapshot = React15.useCallback((snapshot, { restoreProvisioning = false } = {}) => {
    const now2 = Date.now();
  ...
  const loadStatus = React15.useCallback(async ({ signal, silent = false, restoreProvisioning = false } = {}) => {
    const workspaceVersion = workspaceFence.beginStatus();
  ...
  const startProvisioning = React15.useCallback(async ({
    replace = false,
  ...
  const setBotError = React15.useCallback((botId, error) => {
  ...
  const repairCallback = React15.useCallback((connection) => {
  ...
  const reconnectOneBot = React15.useCallback(async (connection) => {
  ...
  const saveWorkspace = React15.useCallback(async (connection, workspace) => {
  ...
  const saveBotSetting = React15.useCallback(async (connection, operation, endpoint, payload) => {
    const { botId } = connection;
    const snapshotVersion = workspaceFence.beginMutation();
  ...
  const requestRemove = React15.useCallback((connection) => {
    setRemoveTargetId(connection.botId);
  }, []);
  const cancelRemove = React15.useCallback(() => {
  ...
  const confirmRemove = React15.useCallback(async (connection) => {
    const { botId, bot } = connection;
    const snapshotVersion = workspaceFence.beginMutation();
  ...
  const setCardRef = React15.useCallback((botId, node) => {
    if (node) cardRefs.current.set(botId, node);
    else cardRefs.current.delete(botId);
  }, []);
  const setRemoveButtonRef = React15.useCallback((botId, node) => {
...
var FEISHU_STYLE_ID = "onlyforchris-dsh-im-feishu-settings";
var CSS3 = String.raw`
.bxf-page {
...
var QQ_STYLE_ID = "onlyforchris-dsh-im-qq-settings";
var CSS4 = String.raw`
...
var Button7 = React16.forwardRef(function Button8({ children, kind = "secondary", className = "", ...props }, ref) {
  return h2("button", {
    ...props,
    ref,
    type: "button",
...
function OfficeSettingsTab({ rpcCall, initialStatus }) {
  const [model, setModel] = React17.useState(normalizeOfficeStatus(initialStatus));
  const [phase, setPhase] = React17.useState(initialStatus === void 0 ? "loading" : "ready");
  const [busy, setBusy] = React17.useState("");
  const [error, setError] = React17.useState("");
  const [notice, setNotice] = React17.useState("");
  const [form, setForm] = React17.useState({
    baseUrl: "",
  ...
  const load = React17.useCallback(async () => {
    try {
...
var OFFICE_STYLE_ID = "onlyforchris-dsh-im-office-settings";
var CSS5 = `
...
var SLACK_STYLE_ID = "onlyforchris-dsh-im-slack-settings";
var CSS6 = String.raw`
...
function SlackCredentialPanel({ busy, error, onSubmit, onCancel }) {
  const [botToken, setBotToken] = React18.useState("");
  const [appToken, setAppToken] = React18.useState("");
  const [copied, setCopied] = React18.useState(false);
  const headingId = React18.useId();
...
var TELEGRAM_STYLE_ID = "onlyforchris-dsh-im-telegram-settings";
var CSS7 = String.raw`
...
var WECOM_STYLE_ID = "onlyforchris-dsh-im-wecom-settings";
var CSS8 = String.raw`
...
var Button10 = React19.forwardRef(function Button11({ children, kind = "secondary", className = "", ...props }, ref) {
  return h2("button", {
    ...props,
    ref,
    type: "button",
...
function WecomSettingsTab({ rpcCall }) {
  ...
  const [credentialOpen, setCredentialOpen] = React19.useState(false);
  const [credentialError, setCredentialError] = React19.useState(null);
  ...
  const noticeFrameRef = React19.useRef(null);
  const announce = React19.useCallback((message) => {
    if (!mounted.current) return;
    if (noticeFrameRef.current !== null) {
      window.cancelAnimationFrame(noticeFrameRef.current);
      noticeFrameRef.current = null;
  ...
  const bindCredentials = React19.useCallback(async ({ identity, secret }) => {
    const snapshotVersion = workspaceFence.beginMutation();
    setBusy(true);
    setCredentialError(null);
    try {
...
var WEIXIN_STYLE_ID = "onlyforchris-dsh-im-weixin-settings";
...
var WHATSAPP_STYLE_ID = "onlyforchris-dsh-im-whatsapp-settings";
...
var IM_STYLE_ID = "onlyforchris-dsh-im-settings";
...
function ManualUpdateCommand({ command, disabled, sourceInstall, desktop }) {
  ...
  return h2(
    ...
    command ? h2(
      ...
      command.endsWith("@onlyforchris/dsh-im@latest") ? h2(
        "p",
